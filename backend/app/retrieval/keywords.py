...
def _gemini_chat(messages: list[dict]) -> str:
    ...
    resp = httpx.post(
        f"https://generativelanguage.googleapis.com/v1beta/models/{settings.retrieval_fts_keyword_model}:generateContent?key={settings.gemini_api_key}",
        json={
...
def extract_fts_keywords(
    ...
    *,
    ...
    try:
        messages = [
            {"role": "system", "content": _SYSTEM_PROMPT + "\n\nRespond ONLY with valid JSON matching: {\"terms\": [\"term1\", \"term2\"]}"},
        ...
        parsed = FtsKeywordExtraction(**parsed_json)
